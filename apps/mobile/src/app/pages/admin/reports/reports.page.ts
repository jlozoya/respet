      ...
      <div class="rs-container">
        <div class="rs-chips filters">
          @for (option of statuses; track option.value) {
            <button type="button" class="rs-chip" [class.active]="status() === option.value" (click)="changeStatus(option.value)">
        ...
        @for (report of reports(); track report.id) {
          <article class="rs-card report">
            <header class="head">
              <span class="kind">{{ 'MODERATION.TARGETS.' + report.targetType.toUpperCase() | translate }}</span>
            ...
            <div class="people">
              @if (report.reporter; as reporter) {
                <span class="person">
                  ...
                  <span class="rs-small">{{ 'MODERATION.REPORTED_BY' | translate: { name: (reporter | fullName) } }}</span>
                </span>
              }
              @if (report.targetOwner; as owner) {
                <a class="person" [routerLink]="['/profile', owner.name]">
                  <app-avatar [user]="owner" [size]="28" />
                  <span class="rs-small">{{ 'MODERATION.AUTHOR' | translate: { name: (owner | fullName) } }}</span>
                </a>
              }
            </div>
            ...
            <footer class="actions">
              @if (link(report); as target) {
                <ion-button size="small" class="rs-soft" [routerLink]="target"><ion-icon slot="start" name="open-outline" /> {{ 'MODERATION.OPEN' | translate }}</ion-button>
              }
              @if (report.status === 'pending') {
                <ion-button size="small" class="rs-soft" (click)="resolve(report, 'dismissed')">{{ 'MODERATION.DISMISS' | translate }}</ion-button>
                <ion-button size="small" (click)="resolve(report, 'reviewed')">{{ 'MODERATION.REVIEWED' | translate }}</ion-button>
              } @else {
...
export class ReportsPage implements OnInit {
  ...
  private async fetch(): Promise<void> {
    ...
    try {
      ...
      const result = await this.moderation.reports({ status: this.status(), page: next, perPage: 20 });
